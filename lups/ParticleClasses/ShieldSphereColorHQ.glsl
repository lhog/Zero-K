...
container.oitFillShaderFragment = [[
	...
	void main() {
		...
		#else
			float opaqueDepth = NORM2SNORM(opaqueDepth);
		#endif
		...
		//color.rgb = vec3(d);
		vec3 noiseVec = worldPos.xyz;
		float normHeight = abs(worldPos.y - translationScale.y) / translationScale.w;
		//normHeight = clamp(normHeight, 0.0, 1.0);
		normHeight = smoothstep(0.0, 1.0, normHeight);

		//noiseVec.y += mix(10.0, 11.0, normHeight) * gameFrame;

		noiseVec *= 0.01;

		noiseVec.y += 0.1 * gameFrame; // * mix(0.9995, 1.0, normHeight);


		color.rgba *= abs(Cellular3D( noiseVec ));
		color.rgba = mix(vec4(0, 0, 0.5, 0.4) ,color.rgba, d*d);

		//color.rgb = vec3(worldPos.y - 32.0) / 350.0;
		//color.a = 1.0;
